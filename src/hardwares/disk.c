...
void write_at(FILE* file, size_t address, BYTE value) {
...
void write_buffer_at(FILE* file, size_t address, const BYTE* buffer, size_t bufferSize) {
    for (size_t i = 0; i < bufferSize; i++) {
        write_at(file, address + i, buffer[i]);
    }
}

BYTE read_at(FILE* file, size_t address) {
    _TRACE("read_at : try to read byte at address %zu from physical disk", address);
    size_t pos = get_byte_offset(file, address);
    fseek(file, pos, SEEK_SET);
    char hex_byte[3] = {0};
    fread(hex_byte, 1, 2, file);
    BYTE c = (BYTE)strtol(hex_byte, NULL, 16);
    _TRACE("read_at : success to read byte at address %zu from physical disk, data is %c", address, c);
    return c;
}

void read_buffer_at(FILE* file, size_t address, BYTE* buffer, size_t bufferSize) {
...
void read_disk(int address, BYTE* buffer, int size){
    int disk_id = 0;
    int remain_address = address;
    for(int i = 0; i < MAX_DISKS; i++){
        disk* d = &physical_disks[i];
        remain_address -= d->size;
        if(remain_address < 0){
            remain_address += d->size;
            break;
        }
    ...
    disk* d = &physical_disks[disk_id];
    read_buffer_at(d->file, remain_address, buffer, size);
}
void write_disk(int address, const BYTE *buffer, int size){
    int disk_id = 0;
    int remain_address = address;
    for(int i = 0; i < MAX_DISKS; i++){
        disk* d = &physical_disks[i];
    ...
    disk* d = &physical_disks[disk_id];
    write_buffer_at(d->file, remain_address, buffer, size);
}